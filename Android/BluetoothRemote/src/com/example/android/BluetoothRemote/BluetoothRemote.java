...
//        });
    ...
    private void sendMessage(String message) {
        ...
        if (message.length() > 0) {
            ...
            if(result)
            {
            if(D) Log.i(TAG, "ir.IrGetVersion: True");
            String strLog=String.format("%d ",version.length,version[0],version[1]);
            if(D) Log.i(TAG, strLog);
            }
            else
            {
              if(D) Log.i(TAG, "ir.IrGetVersion: False");
            }
...
//                	}
    ...
    public void onActivityResult(int requestCode, int resultCode, Intent data) {
        ...
        case REQUEST_CONFIG_REMOTE:
        	if(resultCode==Activity.RESULT_OK)
        	{
        		mCodeNum=data.getIntExtra(ConfigRemote.REMOTE_CODENUMBER,301);
        		setConnectedTitle();
        	}
        	break;
        }
    ...
    public boolean onOptionsItemSelected(MenuItem item) {
	 ...
	 public void onBtnClick(View view) { 
	     // Kabloey
			 soundPool.play(soundId, 14, 14, 1, 0, 1f);// 备注3  
			 byte  keyId   =   Byte.parseByte(view.getTag().toString(),16);
			 boolean result= mmIrController.transmitPreprogramedCode((byte)0x81, (byte)(mCodeNum%10), mCodeNum/10, keyId);
//			 if(D) Log.i(TAG, "onBtnClick: result:" + result);
	 }
}