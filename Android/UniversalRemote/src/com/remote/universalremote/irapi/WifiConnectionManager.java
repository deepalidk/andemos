...
import com.remote.universalremote.activity.DeviceActivity;
 ...
 */
public class WifiConnectionManager extends IIo implements IConnectionManager {
	// Debugging
	 ...
	 */
	public WifiConnectionManager(Handler handler,Context context) {
		 //获取wifi服务
        mmWiFiManager = (WifiManager) context.getSystemService(Context.WIFI_SERVICE);
		mState = STATE_NONE;
		mHandler = handler;
	}
	...
	public synchronized void connect(String deviceAddr) {
		...
		mConnectThread = new ConnectThread(deviceAddr);
	 ...
	 */
	public synchronized void connected(Socket socket,String address) {
		if (D)
			Log.d(TAG, "connected");
		...
		bundle = new Bundle();
		bundle.putString(DeviceActivity.DEVICE_NAME, "WXT-100");
		msg.setData(bundle);
		mHandler.sendMessage(msg);
	...
	private class ConnectThread extends Thread {
		...
		private String mmIpAddress;

		public ConnectThread(String deviceAddr) {
			mmIpAddress = deviceAddr;
		}
		...
		private void createSocket() {
			...
			try {
				tmp = new Socket(mmIpAddress, 0x4000);
		...
		public void run() {
			...
			if (mmSocket==null) {
				...
				WifiConnectionManager.this.start();
				return;
			}

			// Reset the ConnectThread because we're done
			synchronized (WifiConnectionManager.this) {
				mConnectThread = null;
			}

			// Start the connected thread
			connected(mmSocket, mmIpAddress);
		}
